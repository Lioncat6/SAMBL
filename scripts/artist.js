...
async function fetchSpotifyArtist(artist) {
  ...
  if (!data["error"]) {
    ...
    document.getElementById("artistImageContainer").innerHTML =
      '<a href="' +
      spImgUrl +
    ...
    document.getElementById(
      "artistFollowerCount"
    ).innerHTML = `<h2>${spFollowerCount} Followers</h2>`;
    document.getElementById(
      "artistGenres"
    ).innerHTML = `<p>${spGenresString}</p>`;
  ...
  if (!data["error"]) {
    ...
    for (x in data["items"]) {
      ...
      document.getElementById("loadingText").innerHTML =
        "Loading albums from spotify... (" + x + "/" + albumCount + ")";
    }
  } else {
  ...
  while (currentOffset + 50 < albumCount) {
    ...
    if (!data["error"]) {
      for (x in data["items"]) {
        spotifyAlbumList.push(data["items"][x]);
        document.getElementById("loadingText").innerHTML =
          "Loading albums from spotify... (" +
          Number(Number(x) + Number(currentOffset)) +
          "/" +
          albumCount +
          ")";
      }
...
async function downloadMusicBrainzAlbums() {
  ...
  var currentOffset = 0;
  document.getElementById("loadingText").innerHTML =
    "Downloading MusicBrainz Albums 1/2...";
  const response = await fetch(
    "https://musicbrainz.org/ws/2/release?artist=" +
      mbid +
      "&inc=url-rels&fmt=json&offset=" +
      currentOffset
  );
  const data = await response.json();
  if (response.status == 200) {
    ...
    for (x in data["releases"]) {
      ...
      document.getElementById("loadingText").innerHTML =
        "Loading albums from MusicBrainz 1/2... (" + x + "/" + albumCount + ")";
    }
  ...
  while (currentOffset + 15 < albumCount) {
    ...
    if (response.status == 200) {
      ...
      for (x in data["releases"]) {
        mbAlbumList.push(data["releases"][x]);
        document.getElementById("loadingText").innerHTML =
          "Loading albums from MusicBrainz 1/2... (" +
          Number(Number(x) + Number(currentOffset)) +
          "/" +
          albumCount +
          ")";
...
async function downloadMusicBrainzAlbums2() {
  ...
  var currentOffset = 0;
  document.getElementById("loadingText").innerHTML =
    "Downloading MusicBrainz Albums 2/2...";
  const response = await fetch(
    "https://musicbrainz.org/ws/2/release?track_artist=" +
      mbid +
      "&inc=url-rels&fmt=json&offset=" +
      currentOffset
  );
  ...
  if (response.status == 200) {
    ...
    for (x in data["releases"]) {
      mbAlbumList.push(data["releases"][x]);
      document.getElementById("loadingText").innerHTML =
        "Loading albums from MusicBrainz 2/2... (" + x + "/" + albumCount + ")";
    }
  } else if ((data["error"] = "Not Found" || response.status == 404)) {
    dispErr("Musicbrainz artist not found. URL likely malfomed");
  } else {
  ...
  while (currentOffset + 15 < albumCount) {
    ...
    if (response.status == 200) {
      ...
      for (x in data["releases"]) {
        mbAlbumList.push(data["releases"][x]);
        document.getElementById("loadingText").innerHTML =
          "Loading albums from MusicBrainz 2/2... (" +
          Number(Number(x) + Number(currentOffset)) +
          "/" +
          albumCount +
          ")";
...
function processAlbums() {
  ...
  for (x in spotifyAlbumList) {
    ...
    var htmlToAppend =
      ...
      spotifyId +
      '&preferred_vendor=spf" target="_blank"><div>A-tisket</div></a> <a class="harmonyButton" href="https://harmony.pulsewidth.org.uk/release?url='+spotifyUrl+'&spotify=" target="_blank"><div>Harmony</div></a></div>' ;
...
if (spid && mbid) {
  document
    .getElementById("mbURL")
    .setAttribute("href", "https://musicbrainz.org/artist/" + mbid);
  document.getElementById("loadingContainer").innerHTML =
