...
async function downloadMusicBrainzAlbums() {
	...
	const response = await fetch("https://musicbrainz.org/ws/2/release?artist=" + mbid + "&inc=url-rels+recordings+isrc&fmt=json&limit=100&offset=" + currentOffset);
	const data = await response.json();
...
async function downloadMusicBrainzAlbums2() {
	...
	const response = await fetch("https://musicbrainz.org/ws/2/release?track_artist=" + mbid + "&inc=url-rels+recordings+isrc&fmt=json&limit=100&offset=" + currentOffset);
	const data = await response.json();
	if (response.status == 200) {
		console.log(data);
		albumCount = data["release-count"];
